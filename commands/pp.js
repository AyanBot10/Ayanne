    ...
    execute: async (bot, msg) => {
      try {
        ...
        if (msg.reply_to_message && (msg.text.toLowerCase() === 'pp' || msg.text.toLowerCase() === '/pp')) {
          ...
          // If the command is used without replying, provide instructions
          await bot.sendMessage(msg.chat.id, 'To use this command, reply to a user\'s message with "pp" or "/pp".', {
            reply_to_message_id: msg.message_id
          });
        }
      } catch (error) {
